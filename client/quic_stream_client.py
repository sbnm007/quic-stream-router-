...
async def send_streams(host, port):
    ...
    keylog_file = open(keylog_path, "w")

    # Configure QUIC client
    config = QuicConfiguration(is_client=True, alpn_protocols=H3_ALPN)
    config.verify_mode = ssl.CERT_NONE
    config.keylog_file = keylog_file
    config.session_ticket_handler = None  # Ensure fresh handshake
    ...
    async with connect(host, port, configuration=config) as client:
        ...
        keylog_file.close()  # Ensure secrets are flushed
